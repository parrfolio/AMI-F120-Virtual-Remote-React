...
### Dev Env (run latest node -v)

Dev on mac: `npm run dev`

## Start App on RasPi

Remote into machine: `ssh pi@raspberrypi`
...
### To prevent the stepper fuse from blowing
...
### Keep `npm run start` Alive After SSH Disconnects

If you need the jukebox server to keep running when you log out of SSH, start it inside a process that stays alive on the Pi:

```
sudo npm install -g pm2
cd /home/pi/AMI-F120-Virtual-Remote-React
pm2 start "npm run start" --name ami-f120-virtual-remote
pm2 save
pm2 startup   # follow the printed sudo command so it boots automatically
```

Later you can run `pm2 status`, `pm2 logs ami-f120-virtual-remote`, `pm2 restart ami-f120-virtual-remote`, or `pm2 stop ami-f120-virtual-remote` without reopening SSH sessions.
...
I have a custom build for RasPi 5 in vendor folder
